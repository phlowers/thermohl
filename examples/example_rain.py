...
def scn(I=900, pr=0.0, ws=0.0):
    ...
    dt["wa"] = 90.0 * np.ones_like(dt["time"])
    dt["ws"] = np.zeros_like(dt["time"])
    dt["ws"][np.logical_and(dt["time"] >= 7 * 3600.0, dt["time"] < 9 * 3600.0)] = ws
    dt["pr"] = np.zeros_like(dt["time"])
    dt["pr"][np.logical_and(dt["time"] >= 3 * 3600.0, dt["time"] < 5 * 3600.0)] = pr
    return dt
...
if __name__ == "__main__":
    ...
    # input data
    dc = dico()
    dt = scn(I=900, pr=2.0 / 3.6e06, ws=1.5)
    tc = 3600
    ...
    Imax = 900.0
    pr = np.array([2.0, 4.0, 8.0]) / 3.6e06
    ws = np.array([5.0, 10.0, 15.0]) / 3.6
    ...
    for i in range(3):
        ...
        ax.plot(
            ...
            drt["T_avg"],
            c=ci,
            label="p=%.0f mm/h, u=%.1f m/s" % (pr[i] * 3.6e06, ws[i]),
        )

        if i == 2:
            ay.set_title("case p=%.0f mm/h, u=%.1f m/s" % (pr[i] * 3.6e06, ws[i]))
            for j, cl in enumerate(
                ["P_joule", "P_convection", "P_radiation", "P_precipitation"]
            ):
                cj = "C%1d" % (j,)
