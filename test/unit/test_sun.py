...
def test_scalar_input():
    ...
    minute = 30
    second = 45
    longitude_deg = np.deg2rad(45)  # 45 degrees east

    result = utc2solar_hour(hour, minute, second, longitude_deg)
...
def test_edge_cases():
    ...
    minute = 59
    second = 59
    longitude_deg = np.deg2rad(180)  # 180 degrees east
    ...
    minute = 0
    second = 0
    longitude_deg = np.deg2rad(-180)  # 180 degrees west
...
def test_realistic_cases():
    ...
    minute = 59 * np.ones(3)
    second = 59 * np.ones(3)
    longitude_deg = np.deg2rad(
        np.array([2.33472, 7.75, -4.48])
    )  # Paris, Strasbourg, Brest
