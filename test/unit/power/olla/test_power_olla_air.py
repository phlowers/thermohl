...
def test_volumic_mass_scalar():
    ...
    result = Air.volumic_mass(air_temperature_c, altitude_meters)

    assert np.isclose(result, expected, rtol=1e-2)


def test_volumic_mass_array():
    air_temperature_c = np.array([0.0, 25.0, 100.0])
...
def test_kinematic_viscosity_scalar():
    ...
    result = Air.kinematic_viscosity(air_temperature_c, altitude_meters)
