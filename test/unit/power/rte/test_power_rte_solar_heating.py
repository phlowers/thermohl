...
def test_solar_irradiance_positive_altitude():
    ...
    latitude_deg = np.deg2rad(45.0)  # Latitude in radians
    month = 6  # June
    day = 21  # Summer solstice
    hour = 12.0  # Noon

    result = solar_irradiance(latitude_deg, month, day, hour)
...
def test_solar_irradiance_array_input():
    ...
    latitude_deg = np.array([np.deg2rad(45.0), np.deg2rad(55.0)])
    month = np.array([6, 12])
    day = np.array([21, 21])
